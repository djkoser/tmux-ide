...
- **Props** — copied verbatim from the `interface` / `type Props` block in the source. If the component uses `React.forwardRef`, the inner Props type is what is shown. TUI components are deliberately untyped or loosely typed in places — the catalog reflects that, it does not normalize.
...
The catalog is the **what**. The four `skills/port-tui-to-*/SKILL.md` files remain the **how** — they walk through the procedural side of porting and they keep their own (scoped) component mapping tables for the porting workflow they describe. If you are porting a screen, read the skill first; if you just need to know which TUI component renders X, read this catalog.
...
- **CLI primitive:** `cardTop('!', innerW)` + `cardRow(text, innerW)` + `cardBot(innerW)` — TUI CLI ships no dedicated banner glyph; an unlabeled card row is the convention.
...
- **CLI primitive:** (React-only) TUI CLI ports are static — there is no animation diff loop. If you want a CLI progress indicator, render a single `cardRow` with the percentage at draw time.
...
- **CLI primitive:** (React-only) TUI CLI ports are static. `OneLineLoaders.tsx` is the explicit React-side carve-out for spinners.
...
- **CLI primitive:** (React-only) TUI CLI ports are static; no animation diffing.
...
- **Used by:** `<Card title="EXAMPLE">...</Card>` repeated throughout `app/page.tsx`, and `<Card title="TUI CLI / TEMPLATE" mode="left">` in `components/examples/CLITemplate.tsx`.
...
- **Props:**
  ...
  ```
- **Theming tokens:** `--theme-text`, `--theme-line-height-base`, `--font-size`
- **CLI primitive:** (React-only) The TUI CLI framework only ships single-border cards in `scripts/cli/lib/card.js`.
...
- **CLI primitive:** `cardRow('[x] label', innerW)` rendered manually by the template; TUI CLI ships no checkbox primitive yet.
...
- **CLI primitive:** `createApp({ interactive: { count, onSelect } })` plus `cardSelectRow` for each filtered row. TUI CLI's interactive selection lifecycle is the equivalent.
...
- **CLI primitive:** TUI CLI's `createApp({ interactive: { count, onSelect } })` lifecycle plus `cardSelectRow` is the closest equivalent.
...
- **CLI primitive:** (React-only) The CLI templates capture keystrokes through `createApp({ onKey })` and compose their own input strings — TUI CLI ships no boxed input primitive.
...
- **Props:**
  ...
  ```
- **Theming tokens:** (none)
- **CLI primitive:** (React-only) TUI CLI is a single process; there is no provider tree.
...
- **Purpose:** Semantic `<table>` wrapper that renders a `<tbody>` shell for TUI-styled tables.
- **Props:**
  ```ts
